  ...
  try {
    ...
    for (const existingLeave of existingLeaves) {
      ...
      if (
        ...
        console.log(
          `Overlap detected for user ${user_id}: New leave ${startDateObj.toISOString()} to ${endDateObj.toISOString()} overlaps with existing leave ${existingStartDate.toISOString()} to ${existingEndDate.toISOString()} (ID: ${existingLeave.leave_id})`,
        );
        res.status(400).json({
    ...
    const allowedLeaveTypeNamesForRole = (
      constants_1.roleInitialBalances[user_role_id] || []
    ...
    if (is_balance_based && user_role_id !== constants_1.INTERN_ROLE_ID) {
      ...
      const availableDays =
        parseFloat(userBalance.total_days) - parseFloat(userBalance.used_days);
      ...
      console.log(
        `Balance check passed for non-Intern. Available: ${availableDays.toFixed(2)}, Requested: ${requestedDays.toFixed(2)}`,
      );
    } else if (
      is_balance_based &&
      user_role_id === constants_1.INTERN_ROLE_ID
    ) {
...
const updateLeaveStatusHandler = async (req, res) => {
  ...
  if (
    !loggedInUser ||
    !manager_user_id ||
    manager_role_id !== constants_1.MANAGER_ROLE_ID
  ) {
    console.error(
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `Manager ${manager_user_id} attempted to process leave ${leaveId} submitted by user ${leaveRequest.user?.user_id || "N/A"} who does not report to them.`,
      );
      res
        .status(403)
        .json({
    ...
    if (status === "Approved") {
      ...
      if (
        (submittingUserRoleId === constants_1.EMPLOYEE_ROLE_ID ||
          submittingUserRoleId === constants_1.INTERN_ROLE_ID) &&
        leaveDuration > 5
      ) {
