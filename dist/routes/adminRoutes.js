...
const leaveTypeRepository = data_source_1.AppDataSource.getRepository(
  LeaveType_1.LeaveType,
); // Repository for LeaveType entity
const leaveBalanceRepository = data_source_1.AppDataSource.getRepository(
  LeaveBalance_1.LeaveBalance,
); // Get LeaveBalance Repository
...
const leaveRepository = data_source_1.AppDataSource.getRepository(
  Leave_1.Leave,
);
const leaveApprovalRepository = data_source_1.AppDataSource.getRepository(
  LeaveApproval_1.LeaveApproval,
);
  ...
  try {
    ...
    try {
      ...
      const balancesToCreate =
        constants_1.roleInitialBalances[createdUser.role.role_id] || []; // Default to empty array if role not in map
      if (balancesToCreate.length > 0) {
        ...
        if (leaveTypes.length !== leaveTypeNames.length) {
          console.warn(
            `createUserHandler: Could not find all required leave types for role ${role.name}. Missing types: ${leaveTypeNames
        ...
        for (const balanceRule of balancesToCreate) {
          ...
          if (leaveType) {
            ...
            newBalance.available_days = Number(balanceRule.initialDays).toFixed(
              2,
            ); // Ensure number, format to 2 decimals
...
const getUsersHandler = async (req, res) => {
  ...
  const filterRoleId = roleIdParam
    ...
    : undefined; // Use toString() and check isNaN
  try {
    // Build query options
    const findOptions = {
    ...
    if (userIds.length > 0) {
      ...
      const allRelevantBalances = await leaveBalanceRepository.find({
        ...
        select: [
          ...
          "year",
          "leaveType",
        ], // Select fields, including relation
      });
      ...
      for (const user of users) {
        ...
        const formattedBalances = userBalances.map((balance) => ({
          ...
          availableDays:
            parseFloat(balance.total_days) - parseFloat(balance.used_days),
    ...
    console.log(
      `Prepared response for ${usersWithBalances.length} users including balances.`,
    );
...
const getAdminApprovalsHandler = async (req, res) => {
  ...
  const admin_user_role_id = req.user?.role_id;
  console.log(
    `--- Admin user ${admin_user_id} accessing leave requests needing Admin approval ---`,
  );
  ...
  try {
    ...
    console.log(
      `Workspaceed ${leavesNeedingAdminApproval.length} leave requests needing Admin approval.`,
    );
...
const updateLeaveStatusByAdminHandler = async (req, res) => {
  ...
  console.log(
    `--- Admin ${admin_user_id} (Role: ${admin_role_id}) attempting to update status of leave ${leaveId} via /api/admin/leave-requests/:id/status ---`,
  );
  // --- Role Check: Only Admins can use this handler ---
  if (
    !loggedInUser ||
    !admin_user_id ||
    admin_role_id !== constants_1.ADMIN_ROLE_ID
  ) {
    console.error(
  ...
  try {
    ...
    const leaveRequest = await leaveRepository.findOne({
      where: [
        ...
        {
          leave_id: leaveId,
          status: Leave_1.LeaveStatus.Awaiting_Admin_Approval,
        }, // Long Employee/Intern leaves after Manager approval
      ],
    ...
    console.log(
      `Admin ${admin_user_id}: Processing leave ${leaveId} (current status ${oldStatus}), Admin action: ${status}.`,
    );
    ...
    if (status === "Approved") {
      ...
      console.log(
        `Leave ${leaveId} admin approved -> status set to '${newStatus}'.`,
      );
      ...
      try {
        ...
        if (!leaveType) {
          ...
          if (userBalance) {
            ...
            console.log(
              `Admin ${admin_user_id}: Updated leave balance for user ${leaveRequest.user_id}, type ${leaveRequest.type_id}, year ${leaveYear}. Used days now: ${userBalance.used_days}`,
            );
          } else {
            console.error(
          ...
          console.log(
            `Admin ${admin_user_id}: Leave type ${leaveType?.name} is not balance-based. No balance update needed for leave ${leaveId}.`,
          );
        }
      } catch (balanceError) {
      ...
      console.log(
        `Leave ${leaveId} rejected by Admin -> status set to '${newStatus}'.`,
      );
    ...
    if (
      (oldStatus === Leave_1.LeaveStatus.Pending ||
        oldStatus === Leave_1.LeaveStatus.Awaiting_Admin_Approval) &&
      (newStatus === Leave_1.LeaveStatus.Approved ||
        newStatus === Leave_1.LeaveStatus.Rejected)
    ) {
      if (leaveApprovalRepository && admin_user_id) {
        try {
          ...
          newApproval.action =
            newStatus === Leave_1.LeaveStatus.Approved
              ? LeaveApproval_1.ApprovalAction.Approved
          ...
          console.log(
            `Admin ${admin_user_id}: Action '${newApproval.action}' logged for leave ${leaveRequest.leave_id} by approver ${admin_user_id}.`,
          );
        } catch (logError) {
          console.error(
    ...
    console.log(
      `Leave request ${leaveId} final status after Admin processing: ${leaveRequest.status}.`,
    );
...
const deleteLeaveTypeHandler = async (req, res) => {
  ...
  console.log(
    `--- Admin ${admin_user_id} (Role: ${admin_role_id}) attempting to delete leave type ${leaveTypeId} ---`,
  );
  // --- Role Check: Only Admins can use this handler ---
  if (
    !req.user ||
  ...
  try {
    ...
    console.log(
      `Admin ${admin_user_id}: Successfully deleted leave type ${leaveTypeId}.`,
    );
