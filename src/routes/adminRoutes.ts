...
            newBalance.available_days = Number(balanceRule.initialDays).toFixed(2);
...
    const filterRoleId = roleIdParam ? parseInt(roleIdParam.toString(), 10) : undefined;
...
        const currentYear = new Date().getFullYear();
        ...
        if (userIds.length > 0) {
            const allRelevantBalances = await leaveBalanceRepository.find({
                ...
                relations: ["leaveType"],
                select: ["balance_id", "user_id", "total_days", "used_days", "year", "leaveType"]
            });
...
router.get('/leave-requests/approvals-needed', protect, getAdminApprovalsHandler);

const updateLeaveStatusByAdminHandler: RequestHandler<
    { id: string }, // Req Params (expecting leave ID in the URL)
    { message: string, leaveId: number, newStatus: LeaveStatus } | { message: string },
