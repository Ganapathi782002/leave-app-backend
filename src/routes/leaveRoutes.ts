...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `No overlap detected for user ${user_id}. Proceeding with application.`); // --- NEW: Role-based Leave Type Check --- // Assuming Interns (ROLE_ID 4) can ONLY apply for Loss of Pay (need LeaveType ID for this) // Let's assume Loss of Pay has type_id 5 for this example - adjust if different
    // --- End Overlap Validation Check ---

    const allowedLeaveTypeNamesForRole = (roleInitialBalances[user_role_id] || []).map(rule => rule.leaveTypeName);
    ...
    res
      ...
      .json({
        message:
          errorMessage || "Internal server error during leave submission",
...
  {} // Req Query
> = async (req: AuthenticatedRequest, res): Promise<void> => { // Use AuthenticatedRequest type if available
    ...
    if (!loggedInUser || !manager_user_id || manager_role_id !== MANAGER_ROLE_ID) {
    ...
    try {
        ...
        if (status === 'Approved') {
            ...
            if (
                (submittingUserRoleId === EMPLOYEE_ROLE_ID || submittingUserRoleId === INTERN_ROLE_ID) &&
                leaveDuration > 5
            ) {
                ...
                // Use leaveRequest.user_id, leaveRequest.type_id, leaveRequest.start_date (for year), actualWorkingDaysOnLeave (which is leaveDuration here), leaveRequest.leaveType?.is_balance_based
                 ...
                 } catch (balanceError: any) { // Explicitly type balanceError
...
// Alternatively, you could use just '/:id/cancel' if you prefer
// router.put('/:id/cancel', protect, cancelLeaveHandler); // -> /api/leaves/:id/cancel
// I've used /my/:id/cancel to group employee actions under /api/leaves/my
